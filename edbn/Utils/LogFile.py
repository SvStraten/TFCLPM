...
class LogFile:
    ...
    def __init__(self, filename, delim, header, rows, time_attr, trace_attr, activity_attr=None, values=None,
        ...
        if filename is not None:
            ...
            for col_type in self.data.dtypes.items():
    ...
    def create_k_context_trace(self, trace):
        ...
        #contextdata.append(joined_trace, ignore_index=True)
    ...
    def split_months(self, date_format, num_days=1):
        ...
        # Convert column to datetime safely (handles various formats)
        self.contextdata[self.time] = pd.to_datetime(self.contextdata[self.time], errors='coerce')

        # Extract year/month
        self.contextdata["month"] = self.contextdata[self.time].dt.strftime("%Y/%m")
    ...
    def extend_data(self, log):
        ...
        train_logfile.contextdata = pd.concat([self.contextdata, log.contextdata], ignore_index=True)
        train_logfile.categoricalAttributes = self.categoricalAttributes
        train_logfile.numericalAttributes = self.numericalAttributes
        # train_logfile.data = self.data.append(log.data)
        train_logfile.data = pd.concat([self.data, log.data], ignore_index=True)
        train_logfile.k = self.k
